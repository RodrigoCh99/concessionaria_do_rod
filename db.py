...
# função que retorna todos os carros com a vriavel vip = S:
...
# função que retona todas as informações de um carro a partir do id:
...
# função para pesquisar os carros
...
# função que contabiliza quantos carros tem na lista vip
def contador_vip(cursor):
    cursor.execute('select * from carros where vip = "S";')
    quantidade = cursor.fetchall()

    return len(quantidade)

# função que muda o estado dos carros para vip
def change_state(conn, cursor, id, estado):
